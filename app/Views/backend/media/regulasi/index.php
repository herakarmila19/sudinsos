                ...
                new $.fn.dataTable.Buttons(api, {
                    buttons: [{
                        ...
                        action: function(e, dt, node, conf) {
                            window.location.href = '<?= base_url('admin/regulasi/new') ?>';
                        }
                    }, ],
                });
                ...
                <table id="tabel" class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            ...
                            <th>Nama</th>
                            <th>File</th>
                            <th>Total Didownload</th>
                            <th>Tanggal Dibuat</th>
                            <th>Fitur</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($regulasiData as $no => $data) : ?>
                            <tr>
                                <td><?= $no + 1 ?></td>
                                <td><?= $data->title ?></td>
                                <td><a href="<?= base_url('upload/regulasi/' . $data->path_regulasi) ?>" target="_blank">Download</a></td>
                                <td><?= $data->didownload ?></td>
                                <td><?= date('d M Y', strtotime($data->created_date)) ?></td>
                                <td><a href="<?= base_url('admin/regulasi/' . $data->id_regulasi) ?>">Detail</a></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
