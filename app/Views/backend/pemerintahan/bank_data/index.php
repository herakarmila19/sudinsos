                ...
                new $.fn.dataTable.Buttons(api, {
                    buttons: [{
                        ...
                        action: function(e, dt, node, conf) {
                            window.location.href = '<?= base_url('admin/bank-data/new') ?>';
                        }
                    }, ],
                });
        ...
        <div class="card">
            ...
            <div class="card-body">
                <table id="tabel" class="table table-bordered table-striped">
                    ...
                    <tbody>
                        <?php foreach ($bankData as $no => $data) : ?>
                            <tr>
                                <td><?= $no + 1 ?></td>
                                <td><?= $data->nama ?></td>
                                <td><?= $data->jenis ?></td>
                                <td><?= date('d M Y', strtotime($data->created_date)) ?></td>
                                <td><a href="<?= base_url('admin/bank-data/' . $data->id) ?>">Detail</a></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
