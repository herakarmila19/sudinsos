                ...
                new $.fn.dataTable.Buttons(api, {
                    buttons: [{
                        ...
                        action: function(e, dt, node, conf) {
                            window.location.href = '<?= base_url('admin/agenda/new') ?>';
                        }
                    }, ],
                });
        ...
        <div class="card">
            ...
            <div class="card-body">
                <table id="tabel" class="table table-bordered table-striped">
                    ...
                    <tbody>
                        <?php foreach ($agendaData as $no => $data) : ?>
                            <tr>
                                <td><?= $no + 1 ?></td>
                                <td><?= $data->nama_acara ?></td>
                                <td><?= date('d M Y', strtotime($data->tanggal_acara)) ?></td>
                                <td><?= $data->jam_acara . ':' . $data->menit_acara ?></td>
                                <td><?= $data->publish == 0 ? 'Draft' : 'Publish' ?></td>
                                <td><a href="<?= base_url('admin/agenda/' . $data->id_agenda) ?>">Detail</a></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
